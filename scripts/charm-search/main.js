...
var sorting = [];
var rows_list = [];
var uniqueIdentifiers = new Set(); // Set to keep track of unique row identifiers
...
function search(){
    ...
    uniqueIdentifiers.clear(); // Clear previous identifiers
    ...
    result = [...new Set(result.map(item => JSON.stringify(item)))].map(item => JSON.parse(item));
    ...
    for(var i = 0; i < Math.min(result.length, max_results); i++){
        ...
        var rowIdentifier = `${result[i]["Charm Name"]}-${result[i]["Skill 1"]}-${result[i]["Skill 2"]}-${result[i]["Charm Slot"]}-${result[i]["Table No."]}`;
        if (uniqueIdentifiers.has(rowIdentifier)) {
            continue; // Skip adding duplicate rows
        }
        uniqueIdentifiers.add(rowIdentifier);

        var headers = [
            rarity,
            {
               skillValue: getSkill(result[i]["Skill 1"])[1],
               skillName: getSkill(result[i]["Skill 1"])[0]
            },
            {
               skillValue: getSkill(result[i]["Skill 2"])[1],
               skillName: getSkill(result[i]["Skill 2"])[0]
            },
            result[i]["Charm Slot"],
        ...
        for (var i = 0; i < sorting_header.length; i++) {
            if (header_index === 1 || header_index === 2) { // Skills sorting
                if (sort_type === 1) {
                   if (
                      headers[header_index].skillValue > sorting_header[i].skillValue || 
                      (headers[header_index].skillValue === sorting_header[i].skillValue && headers[header_index].skillName > sorting_header[i].skillName)
                   ) {
                      index = i;
                      break;
                   }
...
function reset_inputs(){
    ...
    rows_list = [];
    uniqueIdentifiers.clear(); // Clear the unique identifiers on reset
    search_filter = "";
